...
function DashboardTab() {
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            try {
                // Make a fetch request to your backend to get product data
                const response = await fetch('http://localhost:4500/products/product');
                if (response.ok) {
    ...
    const handleDelete =async (id) => {
        if (window.confirm('Are you sure you want to delete this product?')) {
            try {
                ...
                const response = await fetch(`http://localhost:4500/products/delete/${id}`, {
                    method: 'DELETE'
                });
                if (response.ok) {
                    setProducts(products.filter(product => products.id !== id));
    ...
    {products.map((product, index) => (
        <tr key={index} className="bg-gray-70 border-b  dark:border-gray-700" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
            ...
            <td className="px-6 py-4 font-medium text-black whitespace-nowrap">
                <img className='w-45' src={product.imageUrl} alt="Product" />
            </td>
            <td className="px-6 py-4 text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                {product.title}
            </td>
